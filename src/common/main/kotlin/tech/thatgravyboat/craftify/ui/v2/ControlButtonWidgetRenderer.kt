...
class ControlButtonWidgetRenderer(
    ...
    override fun render(graphics: GuiGraphics, ctx: WidgetRendererContext<Button>, partialTicks: Float) {
        val state = this.state.get() == true
        val texture: ResourceLocation? = ImageCaches.ASSET.get(if (state) selectedTexture else this.texture)
