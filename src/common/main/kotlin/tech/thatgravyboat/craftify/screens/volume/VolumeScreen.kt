...
// TODO remove and replace with cursor screen
class VolumeScreen : WindowScreen(version = ElementaVersion.V2, restoreCurrentGuiOnClose = true) {
    ...
    private fun getVolume(): Float {
        return (Craftify.service?.getState()?.player?.volume?.toFloat()?.div(100f) ?: 0.25f).coerceIn(0f..1f)
    }
    ...
    init {
        ...
        slide.onValueChanged {
            Craftify.service?.setVolume((it*100f).toInt())
            volume = it
        }
    }
    ...
    private fun createVolumeButton() {
        ...
        icon?.onMouseClick {
            ...
            Craftify.service?.setVolume((volume*100f).toInt())
            createVolumeButton()
        }
        icon?.onMouseEnter {
